...
async def fetch_roster(session, url: str):
    ...
    Supports:
      - New Sidearm layout (.s-person-card)
    ...
    # ---------------------------------------------------------------------
    # 1) NEW SIDEARM LAYOUT (used by Alabama & many others)
    #    Uses .s-person-card with text like:
    #    "Jersey Number", "00", "Vic Moten", "Position", "RHP",
    #    "Academic Year", "Fr.", ...
    # ---------------------------------------------------------------------
    def parse_new_sidearm_layout():
        ...
        for card in cards:
            ...
            # Players have "Academic Year" label; coaches/staff do not.
            ...
            # Name is the string right before the "Position" label
            name = ""
            try:
                ...
                # Fallback: if we can't find "Position", skip this card
                continue
        ...
        if found:
            print(f"[debug] parsed {len(found)} players using new .s-person-card layout at {url}")
        return found
    ...
    if blocks:
        ...
        print(f"[debug] parsed {len(players)} players using legacy card layout at {url}")
        return players
